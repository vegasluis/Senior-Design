...
uint8_t mpu6050_start(void){
	...
	if(~res){
		i2c_readRegByte(MPU6050_ADDRESS, MPU6050_RA_WHO_AM_I, &res);
...
void mpu6050_init(void){
	...
	i2c_writeRegByte(MPU6050_ADDRESS, MPU6050_RA_PWR_MGMT_1, 0x00); //exit sleep mode
	i2c_writeRegByte(MPU6050_ADDRESS, MPU6050_RA_CONFIG, 0x01); // LPF, bandwidth = 184(accel) and 188(gyro)
	i2c_writeRegByte(MPU6050_ADDRESS, MPU6050_RA_ACCEL_CONFIG, 0x00); //accel ADC scale: 2 g
	
	i2c_writeRegByte(MPU6050_ADDRESS, MPU6050_RA_INT_ENABLE, 0x00); //enable data ready interrupt
	i2c_writeRegByte(MPU6050_ADDRESS, MPU6050_RA_SIGNAL_PATH_RESET, 0x00); //don't reset signal path
...
void mpu6050_read_gyro_X(uint8_t * buff){
	...
	i2c_readRegByte(MPU6050_ADDRESS, MPU6050_RA_GYRO_XOUT_H, buff);
	i2c_readRegByte(MPU6050_ADDRESS, MPU6050_RA_GYRO_XOUT_L, buff+1);
}
...
void mpu6050_read_gyro_Y(uint8_t * buff){
	...
	i2c_readRegByte(MPU6050_ADDRESS, MPU6050_RA_GYRO_YOUT_H, buff);
	i2c_readRegByte(MPU6050_ADDRESS, MPU6050_RA_GYRO_YOUT_L, buff+1);
}
...
void mpu6050_read_gyro_Z(uint8_t * buff){
	...
	i2c_readRegByte(MPU6050_ADDRESS, MPU6050_RA_GYRO_ZOUT_H, buff);
	i2c_readRegByte(MPU6050_ADDRESS, MPU6050_RA_GYRO_ZOUT_L, buff+1);
}
